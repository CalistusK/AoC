...
arrwa.push(change(ia[0],[0,0],brutea));
arrwb.push(change(ib[0],[0,0],bruteb));

for (let i = 1; i < ia.length; i++)
{
    arrwa.push(change(ia[i],arrwa[i-1],brutea));
}

for (let i = 1; i < ib.length; i++)
{
	arrwb.push(change(ib[i],arrwb[i-1],bruteb));
}

brutal = intersect(brutea,bruteb);
brutal.shift();
for (i in brutal)
{
    let tempa = brutea.indexOf(brutal[i]);
    let tempb = bruteb.indexOf(brutal[i]);
    setans.push(tempa + tempb);
}
console.log(Math.min(...setans));

function intersect(a, b) {
    return a.filter(Set.prototype.has, new Set(b));
}

function change(n,arr,tar)
{
    let hack = [...arr].map(Number);
    val = Number(n.substr(1));
	switch (n.charAt(0))
	{
		case "U":
            for (let i = hack[1]+1; i <= hack[1]+val; i++)
            {
                tar.push(String(hack[0]) + ',' + String(i));
            }
            hack[1] += val;
			return hack;
		case "D":
			...
			return hack;
		case "R":
            for (let i = hack[0]+1; i <= hack[0]+val; i++)
            {
                tar.push(String(i) + ',' + String(hack[1]));
            }
			hack[0] += val;
			return hack;
		case "L":
